...
router.get('/charts-data', function(req, res, next){
  ...
  db.query(sql, [1,2,3], function(err, rs){
    if(err){
      ...
      res.send( {datas:rs[0], datum:rs[1]});
...
router.get('/chart-data', function(req, res, next){
  db.query("select * from Products", function(err, rs){
    if (err){
      res.send(err);
    }
    else{
  ...
  router.get('/chartss', function(req, res, next){
    request('http://localhost:3000/chart-data', function (error, response, body) {
    ...
    router.get('/seares', function(req, res, next){
      res.render('BasicColumnChart.html');
...
router.get('/sale_management', function(req, res, next){
  ...
  request('http://localhost:3000/chart-data', function (error, response, body) {
    if (!error && response.statusCode == 200) {
       var importedJSON = JSON.parse(body);
       res.render('sale_management', {produc: importedJSON});
    }
  })
...
router.post('/add_product', function(req, res, next){
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into Products(prod_name , category_name , stockquantity, unitprice, description) values ('"+pname+"','"+cat+"', '"+quant+"', '"+price+"','"+description+"')", function(err, rs){
      if(err){
        console.log(err);
...
router.post('/inventory-update', function(req, res, next){
  ...
  if(errors.length > 0) {
    res.render('inventory-update', {
      ...
      db.query("select prod_id from Products where prod_name = ?; select sup_id from Supplier where sup_name = ? ; update Products set stockquantity = stockquantity + ? where prod_name = ? ;", [pname, sup, quant, pname], function(err, rs){
        if(err){
          res.send(err);
        }
        ...
        var sid = rs[1][0].sup_id;
      
        db.query("insert into productPurchases(prod_id, sup_id, date ,  quantity, comment) values ('"+pid+"','"+sid+"','"+date+"', '"+quant+"','"+description+"')", function(err, rs){
          if(err){
            console.log(err);
...
router.get('/sales', function(req, res, next){
  ...
  var sql = "select * from Products; select * from Customer;"
  db.query(sql, [1,2], function(err, rs){
    if(err){
      res.send(err);
    }
    else{
...
router.post('/sales', function(req, res, next){
  ...
  if(pname === undefined || !price || !quant ) {
    errors.push({msg: 'Please fill in all required fields'});
  } 
  ...
  if(errors.length > 0) {
    res.render('sales', {
      ...
      db.query("select prod_id from Products where prod_name = ?; select cust_id from Customer where Name = ? ;", [pname, cat], function(err, rs){
        if(err){
          res.send(err);
        }
        ...
        var pid = rs[0][0].prod_id;
        var cid = rs[1][0].cust_id;
      
        db.query("insert into Sales(prod_id, cust_id, date , price, quantity, comments) values ('"+pid+"','"+cid+"','"+date+"', '"+price+"', '"+quant+"','"+description+"')", function(err, rs){
          if(err){
            console.log(err);
...
router.post('/supplier', function(req, res, next){
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into Supplier(sup_name, phone_number , email, address, city, state, country, description) values ('"+name+"','"+pno+"', '"+email+"', '"+addy+"','"+city+"', '"+state+"','"+country+"', '"+description+"')", function(err, rs){
      if(err){
        console.log(err);
...
router.post('/customer', function(req, res, next){
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into Customer(Name, phone_no, email, address, city, state, country) values ('"+name+"','"+pno+"', '"+email+"', '"+addy+"','"+city+"', '"+state+"','"+country+"')", function(err, rs){
      if(err){
        console.log(err);
...
router.post('/rimcalc', function(req, res, next){
  const {date , sales , value, cost, profit} = req.body;
  let errors = [];

  if(!date || !sales || !cost || !profit) {
    errors.push({msg: 'Please fill in all required fields'});
  }
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into rimcalc(date, value, sales, cost, profit) values ('"+date+"','"+value+"', '"+sales+"', '"+cost+"','"+profit+"')", function(err, rs){
      if(err){
        console.log(err);
        req.flash('error', 'Error: Rim Sales not Inserted')
        res.redirect('/rimcalc');
      }
      else{
        req.flash('success_msg', 'Rim Sales Inserted')
        res.redirect('/rimcalc');
       }
...
// Insert Battery Sales
router.get('/battcalc', function(req, res, next){
  res.render('battcalc');
});

router.post('/battcalc', function(req, res, next){
  const {date , value, sales , cost, profit} = req.body;
  let errors = [];

  if(!date || !value || !sales || !cost || !profit) {
    errors.push({msg: 'Please fill in all required fields'});
  }
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into battcalc(date, value, sales, cost, profit) values ('"+date+"','"+value+"', '"+sales+"', '"+cost+"','"+profit+"')", function(err, rs){
      if(err){
        console.log(err);
        req.flash('error', 'Error: Battery Sales not Inserted')
        res.redirect('/battcalc');
      }
      else{
        req.flash('success_msg', 'Battery Sales Inserted')
        res.redirect('/battcalc');
       }
...
// Insert   total Sales
router.get('/totcalc', function(req, res, next){
  res.render('totcalc');
});

router.post('/totcalc', function(req, res, next){
  const {date , tprofit , tsales, shop_expenses, good_transport, school_fees, contribution, coperative_contribution, others, household, salary, net_profit} = req.body;
  let errors = [];

  if(!date || !tprofit || !tsales || !shop_expenses || !good_transport || !school_fees || !contribution || !coperative_contribution || !others || !household || !salary || !net_profit ) {
    errors.push({msg: 'Please fill in all required fields'});
  }
  ...
  if(errors.length > 0) {
    ...
    db.query("insert into totcalc(date , tprofit , tsales, shop_expenses, good_transport, school_fees, contribution, coperative_contribution, others, household, salary, net_profit) values ('"+date+"','"+tprofit+"', '"+tsales+"', '"+shop_expenses+"','"+good_transport+"', '"+school_fees+"','"+contribution+"', '"+coperative_contribution+"','"+others+"', '"+household+"','"+salary+"','"+net_profit+"')", function(err, rs){
      if(err){
        req.flash('error', 'Error: Total Sales not Inserted');
        res.redirect('/totcalc');
      }
      else{
        req.flash('success_msg', 'Total Sales Inserted');
        res.redirect('/totcalc');
       }
    });
  }
...
router.get('/homepage', function(req, res, next){
  var sql = 'SELECT * FROM (SELECT * FROM totcalc ORDER BY tot_id DESC LIMIT 5) sub ORDER BY tot_id ASC; SELECT * FROM (SELECT * FROM battcalc ORDER BY batcalc_id DESC LIMIT 5) sub ORDER BY batcalc_id ASC; SELECT * FROM (SELECT * FROM rimcalc ORDER BY rimcalc_id DESC LIMIT 5) sub ORDER BY rimcalc_id ASC;'
  db.query(sql, [1,2,3], function(err, rs){
    var expenses = rs[0][0].shop_expenses + rs[0][0].good_transport + rs[0][0].contribution + rs[0][0].school_fees + rs[0][0].coperative_contribution + rs[0][0].others
    console.log(expenses);
    if(err){
      req.send(err);
    }
    else{
      console.log(rs[1][0].value); 
      res.render('homepage', {datas: rs[0], datum: rs[1], dat:rs[2], expenses:expenses,  moment:moment})
    }
  });
});
...
router.get('/battreport', function(req, res, next){
  db.query("select * from battcalc", function(err, rs){
    if (err){
      res.send(err);
    }

    else{
      res.render('battreport', {dat:rs, moment:moment});
    }
...
router.get('/rimreports', function(req, res, next){
  db.query("select * from rimcalc", function(err, rs){
    if (err){
      res.send(err);
    }

    else{
      res.render('rimreports', {datum:rs, moment:moment});
    }
...
router.get('/totreports', function(req, res, next){
  db.query("select * from totcalc", function(err, rs){
    if (err){
      res.send(err);
    }

    else{
      res.render('totreports', {datas:rs, moment:moment});
    }
...
router.get('/expreport', function(req, res, next){
  db.query("select * from totcalc", function(err, rs){
    if (err){
      res.send(err);
    }
      ...
      res.render('expreport', {datas:rs, moment:moment});
    }
